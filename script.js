// Código para generar la gráfica con datos específicos.
function createChart(data) {
    ...
    // Create a bar for each incident type.
...
// Datos específicos de incidentes
const data = [
...
// Crear la gráfica y añadirla al HTML
...
function initMap() {
    const map = L.map('grafico2').setView([19.432608, -99.133209], 12); // Centra el mapa en Ciudad de México
...
// Función para determinar el color basado en el valor de incidente_c4
function getColor(incident) {
    ...
    return colors[incident] || "#000000"; // Devuelve negro si el incidente no está en la lista
}
...
// Función para inicializar el mapa
function initMap(id) {
    const map = L.map(id).setView([19.432608, -99.133209], 12); // Centra el mapa en Ciudad de México
...
// Inicializar el mapa para grafico2
const map1 = initMap('grafico2');

// Cargar el archivo CSV y agregar los puntos al mapa para grafico2
...
// Inicializar el mapa para grafico4
const map2 = initMap('grafico4');

// Cargar el archivo CSV y agregar los puntos al mapa para grafico4
d3.csv('datos_fotocivicas.csv').then(data => {
    data.forEach(d => {
        ...
        L.circleMarker([lat, lon], {
            color: '#3388ff', // Color para los puntos de datos_fotocivicas
            radius: 5
